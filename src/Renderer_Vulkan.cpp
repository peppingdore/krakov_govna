...
void Vulkan::init()
{
	...
	make_array(&descriptor_pools,        32, c_allocator);
	make_array(&commands,                32, c_allocator);

	make_array_map(&glyphs,              32, c_allocator);

	make_bucket_array(&glyph_atlasses,   32, c_allocator);

	make_array(&used_uniform_buffer_memory_allocations, 32, c_allocator);
...
void Vulkan::create_pipelines()
{
	// General pipeline
	{
		...
		general_pipeline = create_pipeline({
			.vertex_shader   = shaders.general_vertex,
			.fragment_shader = shaders.general_fragment,
	...
	// Texture pipeline
	{
		...
		texture_pipeline = create_pipeline({
			.vertex_shader   = shaders.texture_vertex,
			.fragment_shader = shaders.texture_fragment,
	...
	// Mask pipeline
	{
		...
		inversed_mask_pipeline = create_pipeline({
			.vertex_shader   = shaders.mask_vertex,
			.fragment_shader = shaders.mask_fragment,
		...
		clear_mask_pipeline = create_pipeline({
			.vertex_shader   = shaders.mask_vertex,
			.fragment_shader = shaders.mask_fragment,
	...
	// Line pipeline 
	{
		...
		line_pipeline = create_pipeline({
			.vertex_shader   = shaders.line_vertex,
			.fragment_shader = shaders.line_fragment,
...
Vulkan::Pipeline Vulkan::create_pipeline(Vulkan::Pipeline_Options options)
{
	ZoneScoped;
	...
	VkViewport viewport = {
		.x = 0,
		.y = 0,
		.width  = (float) renderer.framebuffer_width,
		.height = (float) renderer.framebuffer_height,
		.minDepth = 0.0,
		.maxDepth = 1.0
	};
	...
	VkRect2D scissor_rect = {
		...
		.extent = {
			.width  = (u32) renderer.framebuffer_width,
			.height = (u32) renderer.framebuffer_height,
		},
	};
...
void Vulkan::create_primitives()
{
	...
	// Quad
	{
		const static Vector3 quad_vertices[] = {
			Vector3::make(-1, -1, 0),
			Vector3::make(-1,  1, 0),
			Vector3::make( 1,  1, 0),
			Vector3::make( 1, -1, 0),
		...
		primitives.quad.vertices = Dynamic_Array<Vector3>::from_static_array(quad_vertices);
...
void Mesh::allocate_on_gpu()
{
	...
	// Vertex buffer
	{
		...
		if (vkCreateBuffer(vk.device, &bufferInfo, vk.host_allocator, &vertex_buffer) != VK_SUCCESS)
		...
		void* data;
		vkMapMemory(vk.device, vertex_buffer_memory.device_memory, vertex_buffer_memory.offset, bufferInfo.size, 0, &data);
		memcpy(data, vertices.data, vertices_size);
		vkUnmapMemory(vk.device, vertex_buffer_memory.device_memory);
	}
	...
	// Index buffer
	{
		...
		if (vkCreateBuffer(vk.device, &bufferInfo, vk.host_allocator, &index_buffer) != VK_SUCCESS)
			abort_the_mission(U"Failed to vkCreateBuffer");
		...
		void* data;
		vkMapMemory(vk.device, index_buffer_memory.device_memory, index_buffer_memory.offset, bufferInfo.size, 0, &data);
		memcpy(data, indices.data, indices_size);
		vkUnmapMemory(vk.device, index_buffer_memory.device_memory);
	}

}

void Vulkan::load_shaders()
{
	ZoneScoped;
	...
	load_shader(U"general.vert.spirv", &shaders.general_vertex);
	load_shader(U"general.frag.spirv", &shaders.general_fragment);

	load_shader(U"mask.vert.spirv", &shaders.mask_vertex);
	load_shader(U"mask.frag.spirv", &shaders.mask_fragment);

	load_shader(U"line.vert.spirv", &shaders.line_vertex);
	load_shader(U"line.frag.spirv", &shaders.line_fragment);

	load_shader(U"texture.vert.spirv", &shaders.texture_vertex);
	load_shader(U"texture.frag.spirv", &shaders.texture_fragment);

	Log(U"");
}

void Vulkan::create_white_texture()
{
	ZoneScoped;
...
void Vulkan::create_one_channel_texture(u8* image_buffer, u32 width, u32 height, VkImage* out_image, VkImageView* out_image_view, VkSampler* out_sampler, Vulkan_Memory_Allocation* out_image_memory)
{
	ZoneScoped;
...
void Vulkan::make_sure_texture_is_on_gpu(Texture* texture)
{
	ZoneScoped;
...
void Vulkan::create_vk_instance()
{
	ZoneScoped;
	...
	#ifdef NDEBUG
	static_assert(!parameters.enable_validation_layers);
	#endif

	if (parameters.enable_validation_layers)
	{
		...
		for (const char* str : parameters.validation_layers)
		{
			bool found = false;
...
void Vulkan::find_suitable_gpu_and_create_device_and_queue()
{
	ZoneScoped;
...
VkDescriptorSet Vulkan::get_descriptor_set(VkDescriptorSetLayout descriptor_set_layout)
{
	auto create_new_pool = [&]()
	{
		...
		if (vkCreateDescriptorPool(device, &pool_info, host_allocator, descriptor_pools.add()) != VK_SUCCESS)
			abort_the_mission(U"Failed to vkCreateDescriptorPool");
	};


	if (descriptor_pools.count == 0)
	{
		create_new_pool();
	}
	...
	VkDescriptorSetAllocateInfo allocate_info = {
		...
		.pNext = NULL,
		.descriptorPool = *descriptor_pools[current_descriptor_pool],
	...
	if (result != VK_SUCCESS)
	{
		current_descriptor_pool += 1;
		if (current_descriptor_pool == descriptor_pools.count)
		{
			create_new_pool();
		}
...
void Vulkan::create_main_framebuffer()
{
	ZoneScoped;
...

	{
		...
		i.extent.width  = renderer.framebuffer_width;
		i.extent.height = renderer.framebuffer_height;
		i.extent.depth = 1;
...
void Vulkan::frame_end()
{
...

	{
		if (msaa_samples_count == VK_SAMPLE_COUNT_1_BIT)
		{
			VkImageBlit image_blit = {
				...
				.srcOffsets = {
					...
					{
						.x = renderer.framebuffer_width, .y = renderer.framebuffer_height, .z = 1,
					}
				},
				.dstSubresource = {
					.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
		...
		VkImageCopy image_copy = {
			...
			.extent = { .width = renderer.framebuffer_width, .height = renderer.framebuffer_height, .depth = 1 }
		};
	...
	{
		for (auto& item: used_uniform_buffer_memory_allocations)
		{
			vkDestroyBuffer(device, item.buffer, host_allocator);
			vulkan_memory_allocator.free(item.memory);
		}

		used_uniform_buffer_memory_allocations.clear();
	}
}
...
void Vulkan::transition_image_layout(VkImage image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout)
{
	// ZoneScoped;
...
void Vulkan::copy_buffer_to_image(VkBuffer buffer, VkImage image, u32 width, u32 height, u32 dst_offset_x, u32 dst_offset_y)
{
	// ZoneScoped;
...
VkCommandBuffer Vulkan::begin_single_command_buffer()
{
	// ZoneScoped;
...
void Vulkan::end_single_command_buffer(VkCommandBuffer command_buffer)
{
	// ZoneScoped;
...
Texture_Atlas Texture_Atlas::make(int size)
{
    ...
    vkCreateImage(vk.device, &imageInfo, vk.host_allocator, &image);
    image_memory = vulkan_memory_allocator.allocate_and_bind(image, (Vulkan_Memory_Allocation_Flags) 0, code_location());
    ...
    VkImageView image_view;
    vkCreateImageView(vk.device, &image_view_info, vk.host_allocator, &image_view);
    ...
    vkCreateSampler(vk.device, &sampler_info, vk.host_allocator, &image_sampler);
...
bool Texture_Atlas::put_in(void* image_buffer, int image_width, int image_height, int* out_uv_x_left, int* out_uv_y_bottom)
{
	...
	*out_uv_y_bottom = found_y_bottom;
...

	{
		...
		vkCreateBuffer(vk.device, &create_info, vk.host_allocator, &staging_buffer);
		Vulkan_Memory_Allocation staging_buffer_memory = vulkan_memory_allocator.allocate_and_bind(staging_buffer, VULKAN_MEMORY_SHOULD_BE_MAPPABLE, code_location());

		{
			void* data;
		    vkMapMemory(vk.device, staging_buffer_memory.device_memory, staging_buffer_memory.offset, image_size, 0, &data);
			memcpy(data, image_buffer, image_size);
		    vkUnmapMemory(vk.device, staging_buffer_memory.device_memory);
		}


		vk.transition_image_layout(image, format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
		vk.copy_buffer_to_image(staging_buffer, image, image_width, image_height, found_x_left, found_y_bottom);
		vk.transition_image_layout(image, format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);


		vkDestroyBuffer(vk.device, staging_buffer, vk.host_allocator);
		vulkan_memory_allocator.free(staging_buffer_memory);
	}
